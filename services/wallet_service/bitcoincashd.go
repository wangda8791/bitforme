// Package BitcoinCashd is client librari for BitcoinCashd JSON RPC API
...
// A BitcoinCashd represents a BitcoinCashd client
type BitcoinCashd struct {
	client *rpcClient
}

// New return a new BitcoinCashd
func NewBitcoinCashd(host string, port int, user, passwd string, useSSL bool) (*BitcoinCashd, error) {
	rpcClient, err := newClient(host, port, user, passwd, useSSL)
	if err != nil {
		return nil, err
	}
	return &BitcoinCashd{rpcClient}, nil
}
...
func (b *BitcoinCashd) GetBalance(account string, address string, minconf uint64) (balance float64, err error) {
...
func (b *BitcoinCashd) GetNewAddress(account ...string) (addr string, err error) {
	// 0 or 1 account
	if len(account) > 1 {
...
// GetTransaction returns a BitcoinCashd.Transation struct about the given transaction
func (b *BitcoinCashd) GetTransaction(txid string) (details []structs.TransactionDetails, err error) {
...
func (b *BitcoinCashd) SendToAddress(fromAddress string, toAddress string, amount float64, comment, commentTo string, bInteranl bool) (txID string, err error) {
...
func (b *BitcoinCashd) GetDepositTransactions(last int) (txids []string, latestBlockNumber int64, err error) {

	return
}

// GetNewAddress return a new address for account [account].
func (b *BitcoinCashd) GetMainAddress() (addr string) {
	return ""
}

func (b *BitcoinCashd) GetBlockNumber() (latestBlockNumber int64, err error) {
	return
}
