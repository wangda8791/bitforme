// Package Dogecoind is client librari for Dogecoind JSON RPC API
...
// A Dogecoind represents a Dogecoind client
type Dogecoind struct {
	client *rpcClient
}

// New return a new Dogecoind
func NewDogecoind(host string, port int, user, passwd string, useSSL bool) (*Dogecoind, error) {
	rpcClient, err := newClient(host, port, user, passwd, useSSL)
	if err != nil {
		return nil, err
	}
	return &Dogecoind{rpcClient}, nil
}
...
func (b *Dogecoind) GetBalance(account string, address string, minconf uint64) (balance float64, err error) {
...
func (b *Dogecoind) GetNewAddress(account ...string) (addr string, err error) {
	// 0 or 1 account
	if len(account) > 1 {
...
// GetTransaction returns a Dogecoind.Transation struct about the given transaction
func (b *Dogecoind) GetTransaction(txid string) (details []structs.TransactionDetails, err error) {
...
func (b *Dogecoind) SendToAddress(fromAddress string, toAddress string, amount float64, comment, commentTo string, bInteranl bool) (txID string, err error) {
...
func (b *Dogecoind) GetDepositTransactions(last int) (txids []string, latestBlockNumber int64, err error) {

	return
}

// GetNewAddress return a new address for account [account].
func (b *Dogecoind) GetMainAddress() (addr string) {
	return ""
}

func (b *Dogecoind) GetBlockNumber() (latestBlockNumber int64, err error) {
	return
}
